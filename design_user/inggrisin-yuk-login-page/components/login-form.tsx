...
// Demo list of "registered" numbers. Replace with a real lookup later.
const REGISTERED_NUMBERS = ["08123456789", "081234567890"]
...
export function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    // Simulate verifying the WhatsApp number against registered accounts.
    await new Promise((resolve) => setTimeout(resolve, 1500))

    const normalized = phone.replace(/\s|-/g, "")
    if (REGISTERED_NUMBERS.includes(normalized)) {
      // Success — in a real app, redirect to the dashboard.
      setStatus("idle")
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
      ...
      <p className="text-center text-sm text-muted-foreground">
        Belum punya akun?{" "}
        <a href="#" className="inline-flex items-center gap-1 font-semibold text-primary hover:underline">
